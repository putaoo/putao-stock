...
import com.trade.system.calculate.element.ReportIncrement;
import com.trade.system.calculate.element.StockPrice;
import lombok.Data;
...
public class ExcelExport implements Serializable{
    ...
    public static Response buildResponse(ExcelExport export){
        ...
        response.setCurrentStockPrice(export.getBaseInformation().getCurrentStockPrice());
        ...
        response.setLowerRatio(export.getStockPrice().getLowerRatio());
        response.setIncrement(export.getBaseInformation().getIncrement());
        response.setStockName(export.getBaseInformation().getStockName());
        response.setStockCode(export.getBaseInformation().getStockCode());
        response.setStockPrice(export.getStockPrice().getStockPrice());
        response.setProfitQuarterRatio(export.getIncrementRatio().getProfitQuarterRatio());
        response.setRevenueQuarterRatio(export.getIncrementRatio().getRevenueQuarterRatio());
        response.setStaticPE(export.getBaseInformation().getStaticPE());
        response.setReportProfitIncrementRatio(export.getReportIncrement().getReportProfitIncrementRatio());
        response.setReportRevenueIncrementRatio(export.getReportIncrement().getReportRevenueIncrementRatio());
        response.setLastRevenue(export.getReportIncrement().getLastRevenue());
        response.setLastReportProfit(export.getReportIncrement().getLastReportProfit());
